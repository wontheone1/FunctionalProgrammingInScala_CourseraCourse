...
object Huffman {
  ...
  // Part 1: Basics
    def weight(tree: CodeTree): Int = tree.weight // tree match ...
   ...
   */
    def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = freqs match {
        case List() => List()
        case (char, freq) :: tail => Leaf(char, freq) :: makeOrderedLeafList(tail) sortWith(_.weight < _.weight)
    }
  ...
  else until(predicate, transform)(transform(trees))
   ...
   */
    def convert(tree: CodeTree): CodeTable = {
      def lookUp(subTree: CodeTree, acc: List[Bit]): CodeTable = subTree match {
            case Leaf(char, _) => List((char, acc))
            case Fork(left, right, _, _) => mergeCodeTables(lookUp(left, acc :+ 0), lookUp(right, acc :+ 1))
      }
      lookUp(tree, List())
    }
